...
first_run(){
    ...
    case $temp in
        1)
            sudo docker run -d -p 9100:9100 --name node-exporter bitnami/node-exporter:latest
            echo -e "\nThe ip adress: `ip add | grep enp0s8 | awk 'NR==2 {print $2}' | cut -d"/" -f1`:9100."
            con
            main
        ;;
        2)
            ...
            sudo docker image rm bitnami/node-exporter
            con
            main
        ;;
        0)
