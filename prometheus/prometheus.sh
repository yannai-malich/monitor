...
first_run(){
    ...
    case $temp in
        1)
            sudo docker run -d -p 9090:9090 --name=$docker_name -v /home/yannai/Desktop/prometheus/alert.rules.yml:/etc/prometheus/alert.rules.yml -v /home/yannai/Desktop/prometheus/prometheus.yml:/etc/prometheus/prometheus.yml prom/prometheus
            echo -e "\nThe ip adress: `ip add | grep enp0s8 | awk 'NR==2 {print $2}' | cut -d"/" -f1`:9090."
            con
            main
        ;;
        2)
            clear
            echo -e "Removing $docker_name image.\n"
            sudo docker image rm prom/prometheus
            con
            main
        ;;
        0)
