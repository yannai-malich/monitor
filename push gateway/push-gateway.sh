...
first_run(){
    ...
    case $temp in
        1)
            sudo docker run -d -p 9091:9091 --name=$docker_name prom/pushgateway
            echo -e "\nThe ip adress: `ip add | grep enp0s8 | awk 'NR==2 {print $2}' | cut -d"/" -f1`:9091."
            con
            main
        ;;
        2)
            ...
            sudo docker image rm prom/pushgateway
            con
            main
        ;;
        0)
